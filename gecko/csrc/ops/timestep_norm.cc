...
void TimestepNormCPUFwdImpl(
  ...
  at::parallel_for(0, B, 0, [&](int64_t begin, int64_t end) {
    for (int64_t i = begin; i < end; ++i) {
      ...
      const bool* mask_ptr = padding_mask_data == nullptr ? nullptr : padding_mask_data + i * L;
      ...
      for (int64_t j = 0; j < L; ++j) {
        ...
        for (int64_t k = 0; k < H; ++k) {
          ...
          const auto [_, m1, m2] = utils::WelfordUpdate(m0, u_ptr[k], v_ptr[k], x);
...
void TimestepNormCPUBwdImpl(
    ...
    const torch::Tensor& padding_mask,
    torch::Tensor& X_grad,
    torch::Tensor& prev_mean_grad,
    torch::Tensor& prev_var_grad,
  ...
  at::parallel_for(0, B, 0, [&](int64_t begin, int64_t end) {
    for (int64_t i = begin; i < end; ++i) {
      ...
      for (int64_t j = L - 1; j >= 0; --j) {
        const T* prev_mean_ptr = j == 0 ? prev_mean_data + i * H : mean_ptr + (j - 1) * H;
        const bool mask = mask_ptr != nullptr && mask_ptr[j];
        ...
        for (int64_t k = 0; k < H; ++k) {
          ...
          const T_ACC dv = v_grad_ptr[k] - (T_ACC(0.5) * dy * w * delta * utils::Cube(r));
          const T_ACC du = u_grad_ptr[k] - (w * dy_rstd + coef * dv * (x - prev_u));
          const T_ACC dx = w * dy_rstd + dv * coef * (delta + x - prev_u) + coef * du;
          X_grad_ptr[j * H + k] = mask ? T(0) : static_cast<T>(dx);
          u_grad_ptr[k] = mask ? u_grad_ptr[k] : (T_ACC(1) - coef) * du - coef * delta * dv;
...
void GroupTimestepNormCPUFwdImpl(
  ...
  at::parallel_for(0, B * L * num_groups, 0, [&](int64_t begin, int64_t end) {
    for (int64_t i = begin; i < end; ++i) {
      const auto [m0, m1, m2] = utils::RowwiseMoments<T, T_ACC>(D, X_data + i * D, /*padding_mask=*/nullptr);
  ...
  at::parallel_for(0, B, 0, [&](int64_t begin, int64_t end) {
    for (int64_t i = begin; i < end; ++i) {
      ...
      for (int64_t j = 0; j < L; ++j) {
        const bool mask = mask_ptr != nullptr && mask_ptr[j];
        for (int64_t g = 0; g < num_groups; ++g) {
          const auto [_, m1, m2] = utils::WelfordCombine(m0, u_ptr[g], v_ptr[g], 1, gu_ptr[j * num_groups + g], gv_ptr[j * num_groups + g]);
          const T_ACC rstd = T(1) / std::sqrt(m2 + static_cast<T_ACC>(eps));
          ...
          for (int64_t k = 0; k < D; ++k) {
            ...
            Y_ptr[j * H + g * D + k] = mask ? T(0) : static_cast<T>((x - m1) * rstd * w + b);
          }
        }
        m0 += mask ? 0 : 1;
        count_data[i] = m0;
  ...
  at::parallel_for(0, B, 0, [&](int64_t begin, int64_t end) {
    for (int64_t i = begin; i < end; ++i) {
      ...
      for (int64_t j = L - 1; j >= 0; --j) {
        ...
        for (int64_t g = 0; g < num_groups; ++g) {
          const T_ACC prev_u = j == 0 ? static_cast<T_ACC>(prev_mean_data[i * num_groups + g]) : mean_ptr[(j - 1) * num_groups + g];
          ...
          u_grad_ptr[g] = mask ? u_grad_ptr[g] : c1 * du - T_ACC(2) * c1 * c2 * dv * (ux - prev_u);
...
TimestepNormCPUFwd(
  ...
  torch::Tensor cummean = torch::empty({B, L, H}, prev_mean.options().memory_format(at::MemoryFormat::Contiguous));
  torch::Tensor cumrstd = torch::empty({B, L, H}, prev_var.options().memory_format(at::MemoryFormat::Contiguous));
...
TimestepNormFwd(
  ...
  if (X.device().type() == torch::kCUDA) {
    return TimestepNormCUDAFwd(X, prev_count, prev_mean, prev_var, gamma, beta, padding_mask, eps);
  } else {
    return TimestepNormCPUFwd(X, prev_count, prev_mean, prev_var, gamma, beta, padding_mask, eps);
  }
}
...
TimestepNormBwd(
  ...
  if (X.device().type() == torch::kCUDA) {
    return TimestepNormCUDABwd(Y_grad, mean_grad, var_grad, X, prev_mean, count, mean, var, gamma, padding_mask);
  } else {
    return TimestepNormCPUBwd(Y_grad, mean_grad, var_grad, X, prev_mean, count, mean, var, gamma, padding_mask);
  }
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
GroupTimestepNormCPUFwd(
    const torch::Tensor& X,
    const torch::Tensor& prev_count,
    const torch::Tensor& prev_mean,
    const torch::Tensor& prev_var,
...
GroupTimestepNormFwd(
  ...
  if (X.device().type() == torch::kCUDA) {
    return GroupTimestepNormCUDAFwd(X, prev_count, prev_mean, prev_var, gamma, beta, padding_mask, num_groups, eps);
  } else {
    return GroupTimestepNormCPUFwd(X, prev_count, prev_mean, prev_var, gamma, beta, padding_mask, num_groups, eps);
  }
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
GroupTimestepDecayNormFwd(
    const torch::Tensor& X,
  ...
  if (X.device().type() == torch::kCUDA) {
    return GroupTimestepDecayNormCUDAFwd(X, bos_mask, prev_count, prev_mean, prev_var, gamma, beta, padding_mask, num_groups, beta1, beta2, eps);
  } else {
    throw;
  }
}
...
GroupTimestepNormBwd(
  ...
  if (X.device().type() == torch::kCUDA) {
    return GroupTimestepNormCUDABwd(Y_grad, mean_grad, var_grad, X, prev_mean, count, cummean, cumrstd, gamma, padding_mask, num_groups);
  } else {
    return GroupTimestepNormCPUBwd(Y_grad, mean_grad, var_grad, X, prev_mean, count, cummean, cumrstd, gamma, padding_mask, num_groups);
  }
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
GroupTimestepDecayNormBwd(
    const torch::Tensor& Y_grad,
    const torch::Tensor& mean_grad,
    const torch::Tensor& var_grad,
  ...
  if (X_or_Y.device().type() == torch::kCUDA) {
    return GroupTimestepDecayNormCUDABwd(Y_grad, mean_grad, var_grad, X_or_Y,
                                         prev_count, bos_mask, cummean, cumrstd, gamma, beta,
                                         padding_mask, num_groups, beta1, beta2, eps,
                                         memory_efficient);
  } else {
    throw;
  }
...
void DefineTimestepNormOp(py::module& m) {
  m.def("timestep_norm_fwd", &TimestepNormFwd, "TimestepNorm forward")
      ...
      .def("group_timestep_decay_norm_fwd", &GroupTimestepDecayNormFwd,
           "GroupTimestepDecayNorm forward")
      .def("group_timestep_decay_norm_bwd", &GroupTimestepDecayNormBwd,
           "GroupTimestepDecayNorm backward");
}
