...
__global__ void AttentionSoftmaxFwdKernel(
    int64_t outer_size, int64_t inner_size, bool use_causal_mask, const T* x, T* y) {
...
template <typename T, typename T_ACC, int64_t kCapacity, int64_t kNumThreads>
__global__ void MultiSegAttentionSoftmaxFwdKernel(
    int64_t outer_size, int64_t inner_size, int64_t num_heads, bool use_causal_mask,
    const T* x, T* y, const int64_t* q_idx, const int64_t* k_idx) {
  constexpr int64_t kElementsPerThread = kCapacity / kNumThreads;
  constexpr T_ACC kInf = std::numeric_limits<T_ACC>::infinity();
  ...
  // blockDim.x = outer_size , blockDim.y = batch_size
  const int64_t i = blockIdx.y * outer_size + blockIdx.x;
  const int64_t b = blockIdx.y / num_heads;
  const int64_t q = b * outer_size + blockIdx.x;
  const int64_t k = b * inner_size + threadIdx.x;
  const int64_t r = blockIdx.x;
  // num of elements to load
  const int64_t n = use_causal_mask ? inner_size - outer_size + r + 1 : inner_size;
  T_ACC d = T_ACC(0);
  T_ACC m = -kInf;  // attn mask is -inf.

  register_utils::LoadImpl<T, T_ACC, kElementsPerThread>(x + i * inner_size, n, -kInf, x_acc);

#pragma unroll
  for (int64_t j = 0; j < kElementsPerThread; ++j) {
    if(q_idx[q] != k_idx[k + j * kNumThreads])
      x_acc[j] = -kInf;
  }
...
#pragma unroll
  ...
  register_utils::SaveImpl<T_ACC, T, kElementsPerThread>(x_acc, inner_size, y + i * inner_size);
}
...
template <typename T, typename T_ACC, int64_t kCapacity, int64_t kNumThreads>
__global__ void MultiSegAttentionDropKeySoftmaxFwdKernel(
    at::PhiloxCudaState philox_args, int64_t outer_size, int64_t inner_size, int64_t num_heads,
    T_ACC dropout, bool use_causal_mask, const T* x, T* y, const int64_t* q_idx, const int64_t* k_idx) {
  constexpr int64_t kElementsPerThread = kCapacity / kNumThreads;
  constexpr int64_t kCapacityPerThread =
      std::max(kElementsPerThread, random_utils::kRandomUnroll);
  constexpr T_ACC kInf = std::numeric_limits<T_ACC>::infinity();
